...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('lanouehome-order-systemid', systemid);
		$.cookie('lanouehome-order-systemname', systemname);
		$.cookie('lanouehome-order-systemtitle', systemtitle);
	});
	// 显示菜单
	$('.system_menus').hide(0, function () {
		$('.system_' + 1).show();
	});
	$('body').attr('id', 'lanouehome-ucenter');
	$('#system_title').text('用户管理系统');
});
// iframe高度自适应
